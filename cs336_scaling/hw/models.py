...
if __name__ == "__main__":
  ...
  preds_n = pd.DataFrame({"compute_budget": pred_cbs, "parameters": [fit_n(c) for c in pred_cbs]})
  print(preds_n)
  plot_against_compute(min_df, preds_n, "parameters", "Parameters", "figs/3a_params.png")

  min_d_df, fit_d = fit_data(fit_df)
  pred_cbs = sorted(set(cbs) | set(min_d_df["compute_budget"]))
  preds_d = pd.DataFrame({"compute_budget": pred_cbs, "d": [fit_d(c) for c in pred_cbs]})
  print(preds_d)
  plot_against_compute(min_d_df, preds_d, "d", "Data Size", "figs/3b_data.png")
